...
float hue(float s, float t, float h) {
    h = mod(h, 1.0);
    if (h < 0.0) h += 1.0;
    if (h < 1.0 / 6.0) return s + (t - s) * 6.0 * h;
    if (h < 1.0 / 2.0) return t;
    if (h < 2.0 / 3.0) return s + (t - s) * (2.0 / 3.0 - h) * 6.0;
    return s;
}
...
vec4 RGB(vec4 c) {
    if (c.y == 0.) return vec4(vec3(c.z), c.a);
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    SAT.g = 0.5;
    float width = (debuff.g != 0.0) ? 0.1 : 0.0;
    bool test = (uv.x + uv.y > 1. - width && uv.x + uv.y < 1. + width) || ((1. - uv.x) + uv.y > 1. - width && (1. - uv.x) + uv.y < 1. + width);
    if (test) {
        SAT.r = 1.;
        SAT.g = 0.7;
        SAT.b = 0.8 * SAT.b;
