...
- [x] **15. `apply_holo_effect()`** — `gfx_apply_holographic()` at `graphics_effects.c:284` implements time + card-seed phase offset (`holo_g * 7.221f + time`) and a 3-oscillator field; hue-rotation via `gfx_hue()`/`gfx_RGB_to_HSL`. No separate hue-rotation LUT is used.
...
- [x] **17. Polychrome palette cycling** — `gfx_apply_polychrome()` at `graphics_effects.c:404` drives time-variation via `po_g * 2.221f + time` — same shared-oscillator pattern as holographic, higher field frequency. *No per-frame palette-index discrete cycle counter is wired; the variation is continuous rather than frame-stepped through a fixed palette set.*
...
- [x] **21. Use lookup tables for sin/cos and dissolve threshold comparisons** — No sin/cos LUT exists anywhere in the effects pipeline; all calls go to libm `sinf/cosf/sqrtf`. No precomputed noise texture is used — `gfx_dissolve_mask_alpha()` evaluates the noise field per-pixel. *No `sin_lut[256]` or equivalent replacement has been introduced.*
