...
--------------------------------------
1. Implement the `audio.c` fixes and add `logs/audio.log` instrumentation; verify on-device.
2. Replace full dcache flushes with range-scoped writebacks and increase `MAX_QUADS`.
3. Add a CI job (label-triggered) to regenerate artifacts in a pinned container and upload artifacts for review.
4. Create `docs/PSP3000_CHECKLIST.md` (device tests) and add the profiling harness per `DEVGUIDE_PSP3000.md` suggestions.
