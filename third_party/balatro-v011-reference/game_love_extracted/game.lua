...
function Game:start_up()
    ...
    if _QUE_SHADER then
        for k, shader_name in ipairs(self.PORT_CORE_SHADERS) do
            boot_timer('shaders', shader_name,0.4)
            print('Compiling shader: ' .. shader_name)
            self.SHADERS[shader_name] = love.graphics.newShader("resources/shaders/"..shader_name..".fs")
        end
    end
    ...
    boot_timer('load self', 'load self',0.7)
    love.joystick.loadGamepadMappings("resources/gamecontrollerdb.txt")
    if self.F_RUMBLE then
        local joysticks = love.joystick.getJoysticks()
        boot_timer('get joystick', 'get joystick',0.7)
        if joysticks then
            boot_timer('if joy', 'if joy',0.7)
            if joysticks[1] then
...
function Game:set_render_settings()
    ...
    if not _QUE_MIPMAPS then
        ...
        for i=1, #self.asset_atli do
            self.ASSET_ATLAS[self.asset_atli[i].name] = {}
            self.ASSET_ATLAS[self.asset_atli[i].name].name = self.asset_atli[i].name
            self.ASSET_ATLAS[self.asset_atli[i].name].image = love.graphics.newImage(self.asset_atli[i].path, {mipmaps = true})
            self.ASSET_ATLAS[self.asset_atli[i].name].type = self.asset_atli[i].type
            self.ASSET_ATLAS[self.asset_atli[i].name].px = self.asset_atli[i].px
            self.ASSET_ATLAS[self.asset_atli[i].name].py = self.asset_atli[i].py
        end
        for i=1, #self.asset_images do
            self.ASSET_ATLAS[self.asset_images[i].name] = {}
            self.ASSET_ATLAS[self.asset_images[i].name].name = self.asset_images[i].name
            self.ASSET_ATLAS[self.asset_images[i].name].image = love.graphics.newImage(self.asset_images[i].path, {mipmaps = true})
            self.ASSET_ATLAS[self.asset_images[i].name].type = self.asset_images[i].type
            self.ASSET_ATLAS[self.asset_images[i].name].px = self.asset_images[i].px
            self.ASSET_ATLAS[self.asset_images[i].name].py = self.asset_images[i].py
        end
...
function Game:splash_screen()
    ...
    if G.SETTINGS.skip_splash == 'Yes' or not _QUE_SPLASH then
        G:main_menu()
        return
    end
...
function Game:update(dt)
    ...
    if _QUE_SOUND then modulate_sound(dt) end
...
function Game:draw()
    ...
    love.graphics.push()
        ...
        --love.graphics.print('Memory actually used (in kB): ' .. collectgarbage('count'), 10,60)
    love.graphics.pop()
...
love.graphics.pop()
    ...
    if (not G.recording_mode or G.video_control )and _QUE_SHADER then
