...
function Game:set_globals()
    ...
    --         VITA PORT CONFIG
    --||||||||||||||||||||||||||||||
    self.PORT_VITA_WIDTH = 960
    self.PORT_VITA_HEIGHT = 544

    self.PORT_CORE_SHADERS = {
        'dissolve',
        'flame',
    }

    self.PORT_ENABLED_SHADERS = {
        'dissolve',
        'flame',
        'debuff',
        'played',
    ...
    self.F_SKIP_TUTORIAL = _QUE_SKIPTUTORIAL--*Completely skip the tutorial on fresh save
    self.F_BASIC_CREDITS = true             --*Remove references to Daniel Linssens itch.io
    self.F_EXTERNAL_LINKS = true            --Remove all references to any external links (mainly for console)
    self.F_ENABLE_PERF_OVERLAY = true       --*Disable debugging tool for performance of each frame
    self.F_NO_SAVING = false                --Disables all 'run' saving
    self.F_MUTE = not _QUE_SOUND                --*Force mute all sounds
    ...
    self.F_NO_ACHIEVEMENTS = true           --*Disable achievements
    ...
    self.F_PS4_PLAYSTATION_GLYPHS = true   --use PS4 glyphs instead of PS5 glyphs for PS controllers
