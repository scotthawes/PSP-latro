...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv) {
    ...
    if (tex.a > 0.01 && burn_colour_1.a > 0.01 && !shadow && res < adjusted_dissolve + 0.8 * (0.5 - abs(adjusted_dissolve - 0.5)) && res > adjusted_dissolve) {
        tex.rgba = (res < adjusted_dissolve + 0.5 * (0.5 - abs(adjusted_dissolve - 0.5))) ? burn_colour_1.rgba : burn_colour_2.rgba;
    }
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    float delta = max(max(tex.r, max(tex.g, tex.b)) - min(tex.r, min(tex.g, tex.b)), min(tex.r, min(tex.g, tex.b)) * 0.7);
    float fac = 0.8 + 0.9 * sin(13. * uv.x + 5.32 * uv.y + booster.r * 12. + cos(booster.r * 5.3 + uv.y * 4.2 - uv.x * 4.));
    float maxfac = 0.6 * max(fac, 0.);
    tex.rgb = tex.rgb * 0.5 + vec3(0.4, 0.4, 0.8);
    tex.r = tex.r - delta + delta * maxfac * 0.7 - 0.1;
    tex.g = tex.g - delta + delta * maxfac * 0.7 - 0.1;
    tex.b = tex.b - delta + delta * maxfac * 0.7 - 0.1;
    tex.a = tex.a * (0.8 * max(min(1., max(0., 0.3 * max(min(tex.r, min(tex.g, tex.b)) * 0.2, delta) + min(max(maxfac * 0.1, 0.), 0.4))), 0.) + 0.15 * maxfac * (0.1 + delta));
