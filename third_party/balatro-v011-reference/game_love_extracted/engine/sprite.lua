...
function Sprite:draw_shader(_shader, _shadow_height, _send, _no_tilt, other_obj, ms, mr, mx, my, custom_shader, tilt_shadow)
    ...
    for k, allowed_shader in pairs(G.PORT_ENABLED_SHADERS) do
        if _shader == allowed_shader then
            good = true
            break
        end
    end
    ...
    if good and not G.SHADERS[_shader] then
        love.graphics.setCanvas()
        love.graphics.clear()
        love.graphics.print('COMPILING SHADER: ' .. _shader .. " (please wait :D)", G.PORT_VITA_WIDTH/2 - 220, G.PORT_VITA_HEIGHT/2 - 50)
        love.graphics.present()

        G.SHADERS[_shader] = love.graphics.newShader("resources/shaders/".._shader..".fs")
    end
