...
function DisplayInfo(inputText)
  -- Get the current time in a formatted string
  local currentTime = os.date("%Y-%m-%d %H:%M:%S")
  
  -- Print the input text and current time on the top-left corner of the screen
...
function boot_timer(_label, _next, progress)
  ...
  if G.F_VERBOSE then love.graphics.print("LOADING: ".._next, realw/2 - 150, realh/2 +40) end

  local timeElapsed = love.timer.getTime() - (G.ARGS.bt or love.timer.getTime())
  love.graphics.setColor(1, 1, 1, 1)
  love.graphics.print(string.format("Elapsed: %.1f", timeElapsed), realw/2 - 150, realh/2 - 40)
  love.graphics.pop()
  love.graphics.present()
...
function play_sound(sound_code, per, vol)
  ...
  if sound_code and G.SETTINGS.SOUND.volume > 0.001 then
    ...
    if G.F_SOUND_THREAD and _QUE_SOUND then
...
function PLAY_SOUND(args)
  if not _QUE_SOUND then return end
